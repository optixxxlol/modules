...
do
    ...
    function library:New(info)
        ...
        function window:Notification(info)
            local notificationTable = {drawings = {}}
            local notificationAmount = #notifications+1
            --
            local info = info or {}
            ...
            --
            local notification_outline = utility:Create("Frame", {Vector2.new(-text_bounds.X-20,38/4+27*notificationAmount)}, {
                Size = utility:Size(0, text_bounds.X+20, 0, 21),
                Position = utility:Position(0, -text_bounds.X-20, 0, 38/4+27*notificationAmount),
                Transparency = 1,
                Hidden = true,
                ZIndex = 60,
                Color = theme.outline,
                Visible = true
            });notificationTable.outline = notification_outline
            --
            ...
            --
            function notificationTable:Fade(visibility)
            ...
            --
            function notificationTable:Move(vector, speed)
            ...
            --
            function notificationTable:Remove()
            ...
            --
            function notificationTable:UpdateSize()
            ...
            --
            function notificationTable:UpdatePosition(xPos, yPos)
            ...
            --
            notificationTable:UpdateSize()
            --
            notifications[notificationAmount] = notificationTable
            --
            notificationTable:Fade(false)
            --
            wait(0.5)
            --
            notificationTable:UpdatePosition(0, 38/4+27*notificationAmount)
            --
            spawn(function()
                spawn(function()
                    notificationTable:Move(utility:Position(0, 100, 0, notification_outline.Position.Y), 0.2)
                    wait(0.21)
                    notificationTable:UpdatePosition(100, notification_outline.Position.Y)
                end)
                --
                notificationTable:Fade(true)
                --
                wait(1.5^2)
                --
                spawn(function()
                    notificationTable:Move(utility:Position(0, 0, 0, notification_outline.Position.Y), 0.2)
                end)
                --
                notificationTable:Fade(false)
                --
    ...
    function sections:Textbox(info)
        ...
        local section = self
        --
        local textbox = {focused = false, axis = section.currentAxis}
        --
        ...
        local textbox_value = utility:Create("TextLabel", {Vector2.new((math.sqrt(theme.textsize)-0.1)*string.len(def)+3,1), textbox_frame}, {
            ...
            OutlineColor = theme.textborder,
            Center = true,
            Position = utility:Position(0.5, 0, 0, 1, textbox_frame),
            Visible = page.open
        }, section.visibleContent)
        --
        utility:LoadImage(textbox_gradient, "gradient", "https://i.imgur.com/5hmlrjX.png")
        --
        function textbox:UpdatePosition(sign)
            local sign = sign or 1
            --
            textbox_value.Position = textbox_value.Position + (Vector2.new(math.sqrt(theme.textsize)-0.1, 0)*sign)
        ...
        library.began[#library.began + 1] = function(Input)
            ...
            elseif Input.UserInputType == Enum.UserInputType.Keyboard and textbox.focused then
                ...
                elseif Input.KeyCode == Enum.KeyCode.Backspace and textbox_value.Text ~= "" then
                    ...
                    textbox_value.Position = textbox:UpdatePosition(-1)
                else
                    local allowed = false
                    local character = nil
                    --
