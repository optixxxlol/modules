--// CUSTOM DRAWING
-- DRAWING EXTENSION AND CODE BY VOZOID
local drawing = {} do
...
local library = utility.table({folder = "centity", extension = "cent", flags = {}, open = true, keybind = Enum.KeyCode.RightShift, mousestate = services.InputService.MouseIconEnabled, cursor = nil, holder = main, connections = {}}, true)
...
function library.createcolorpicker(default, parent, count, flag, callback, pickers)
    local icon = utility.create("Square", {
        ...
        Position = UDim2.new(1, -22 - (count * 18) - (count * 6), 0, 2),
        ZIndex = 8
    })

    utility.outline(icon, Color3.fromRGB(35,33,35))
    ...
    local window = utility.create("Square", {
        ...
        Size = UDim2.new(0, 192, 0, 150),
        Visible = false,
        Position = UDim2.new(1, -192 + (count * 18) + (count * 6), 1, 6),
        ZIndex = 11
    })

    utility.outline(window, Color3.fromRGB(35,33,35))
    ...
    local saturation = utility.create("Square", {
        ...
        Parent = window,
        Color = default,
        Size = UDim2.new(0, 158, 0, 139),
        Position = UDim2.new(0, 6, 0, 6),
        ZIndex = 14
    })

    utility.outline(saturation, Color3.fromRGB(35,33,35))
    ...
    utility.outline(saturationpicker, Color3.fromRGB(35,33,35))
    ...
    local hueframe = utility.create("Square", {
        ...
        Parent = window,
        Color = default,
        Size = UDim2.new(0, 14, 0, 139),
        Position = UDim2.new(1, -20, 0, 6),
        ZIndex = 14
    })

    utility.outline(hueframe, Color3.fromRGB(35,33,35))
    ...
    utility.outline(huepicker, Color3.fromRGB(35,33,35))
...
function library.createkeybind(default, parent, blacklist, flag, callback)
    ...
    utility.outline(keybutton, Color3.fromRGB(35,35,35))
    ...
    local function set(newkey)
        ...
        if newkey ~= nil and not table.find(blacklist, newkey) then
            ...
            keybutton.Size = UDim2.new(0, sizeX, 0, 14)
...
function library:Window(options)
    ...
    local main = utility.create("Square", {
        ...
        Position = utility.getcenter(600,700),
        Size = UDim2.new(0,600,0,700)
    })
    self.holder = main
    utility.outline(main,options.accent or options.Accent)
    local holder = utility.create("Square", {
        Filled = true,
        Color = Color3.fromRGB(30, 30, 30),
        Position = UDim2.new(0.025, 0,0.043, 0),
        Size = UDim2.new(0, 570,0, 655),
        Parent = main,
        ZIndex = 2
    })
    local holderoutline1 = utility.outline(holder, Color3.fromRGB(26, 24, 26))
    local holderoutline2 = utility.outline(holderoutline1, Color3.fromRGB(36,36,36))
    utility.dragify(main)
    local title = utility.create("Text", {
        Text = options.name or options.Name,
        Color = Color3.new(1,1,1),
        Size = 13,
        Font = 2,
        Outline = true,
    ...
    local holderline = utility.create("Square", {
        Filled = true,
        Color = options.Accent or options.accent,
    ...
    local tabholder = utility.create("Square", {
        Filled = true,
        Color = holder.Color,
        Position = UDim2.new(0.0017, 1,0.005, 0),
        Size = UDim2.new(1, -2,-0.015, 35),
        Parent = holder,
        ZIndex = 2
    })
    ...
    services.RunService.RenderStepped:Connect(function()
        if self.open then
            ...
            cursor.PointB = Vector2.new(mousepos.X + 16, mousepos.Y + 6)
            cursor.PointC = Vector2.new(mousepos.X + 6, mousepos.Y + 16)
            cursoroutline.PointA = Vector2.new(mousepos.X, mousepos.Y)
            cursoroutline.PointB = Vector2.new(mousepos.X + 16, mousepos.Y + 6)
            cursoroutline.PointC = Vector2.new(mousepos.X + 6, mousepos.Y + 16)
        end
    end)
    --
    local windowtypes = utility.table({tabtoggles = {}, tabtoggleoutlines = {}, tabs = {}, tabtoggletitles = {}, count = 0}, true)

    function windowtypes:Tab(taboptions)
        local tabOptions = taboptions
        ...
        local title = utility.create("Text", {
            Text = taboptions.name or taboptions.Name,
            Font = 2,
            Color = Color3.fromRGB(255,255,255),
            Size = 13,
            Parent = tabtoggle,
        ...
        local tab = utility.create("Square", {
            ...
            Parent = holder,
            ZIndex = 2,
            Position = UDim2.new(0.012, 0,0.063, 0),
            Size = UDim2.new(0, 556,0, 605),
            Visible = false
        })
        ...
        tabtoggle.MouseButton1Down:Connect(function()
            tabtoggle.Transparency = tab.Visible == true and 0 or 1
        end)
        ...
        function tabtypes:Section(options)
            ...
            local sectionline = utility.create("Square", {
                Filled = true,
                Thickness = 0,
                Size = UDim2.new(1,0,0, 2),
                Position = UDim2.new(0,0,0,0),
                Parent = section,
                Color = holderline.Color,
                ZIndex = 7
            })

            local out1 = utility.outline(section, Color3.fromRGB(16,16,16))
            local out2 = utility.outline(out1, Color3.fromRGB(35,33,35))
            ...
            function sectiontypes:Button(options)
                ...
                utility.outline(button, Color3.fromRGB(35,33,35))
            ...
            function sectiontypes:Toggle(options)
                ...
                holder.MouseButton1Down:Connect(function()
                    if not toggled then
                        icon.Color = Color3.fromRGB(25,25,25)
                    end
                end)
                ...
                local function setstate()
                    ...
                    icon.Color = holderline.Color
                    icon.Color = toggled and holderline.Color or Color3.fromRGB(25,25,25)
            ...
            function sectiontypes:Box(options)
                ...
                utility.outline(box, Color3.fromRGB(35,35,35))
            ...
            function sectiontypes:Slider(options)
                ...
                local text = options.text or ("[value]"..suffix)
                ...
                utility.outline(slider, Color3.fromRGB(35,35,35))
                ...
                utility.connect(slider.InputBegan, function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        sliding = true
                        slider.Color = Color3.fromRGB(25,25,25)
                        slide(input)
                    end
                end)
            ...
            function sectiontypes:Dropdown(options)
                ...
                if type(default) == "table" then
                    if max then
                        for i, opt in next, default do
                            if not table.find(values, opt) then
                ...
                elseif default ~= nil then
                    if not table.find(values, default) then
                        default = nil
                    end
                end
                ...
                utility.outline(dropdown, Color3.fromRGB(35,35,35))
                ...
                utility.outline(contentframe, Color3.fromRGB(35,35,35))
                ...
                set = function(option)
                    if max then
                        ...
                        for i, opt in next, option do
                            if table.find(values, opt) and #chosen < max then
